...
loader = Data.DataLoader(
    MyDataSet(enc_inputs, dec_inputs, dec_outputs), 2, True)  # batch_size = 2, shuffle = True
# 所有的数据都已经存在了里面，是一对对相互匹配起来的句子对。
...
class PositionalEncoding(nn.Module):  # 位置编码
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)  # [[0.],[1.],[2.],[3.],[4.]]
        div_term = torch.exp(torch.arange(
            0, d_model, 2).float() * (-math.log(10000.0) / d_model))  # 输出是一个d_model/2大小的张量列表
        pe[:, 0::2] = torch.sin(position * div_term)  # 偶数列位置编码用sin，是直接切片，然后相当于对其中的一半进行了赋值
        pe[:, 1::2] = torch.cos(position * div_term)  # 奇数列位置编码用cos，是直接切片，然后相当于对其中的一半进行了赋值
        pe = pe.unsqueeze(0).transpose(0,
                                       1)  # unsqueeze在张量前添加一个维度，此时变为了1*max_len*d_model的尺寸，然后transpose将1和max_len进行了调换。
        self.register_buffer('pe',
                             pe)  # register_buffer方法用于注册一个非训练参数的Tensor，parameters()不会返回它们，但是需要保存，如位置编码positional encoding.

    def forward(self, x):
        """
        x: [seq_len, batch_size, d_model]
        """
        x = x + self.pe[:x.size(0), :]  # 每个样本的长度是超不过你设置的最大长度5000的，因此只需要把对应部分的编码添加上即可。
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    # [batch_size, 1, len_k], True is masked，最后的这个pad_attn_mask是一个布尔三维矩阵
    pad_attn_mask = seq_k.data.eq(0).unsqueeze(1)  # 在最新版中如果想要获取不包含梯度的张量值可以用detach(),最后在第二个维度处新增一个维度。
    # [batch_size, len_q, len_k] 构成一个立方体(batch_size个这样的矩阵)
    return pad_attn_mask.expand(batch_size, len_q, len_k)  # 广播操作，并不是添加操作。
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self):
        ...
        self.W_K = nn.Linear(d_model, d_k * n_heads, bias=False)  # d_k*n_heads为d-model，而且注意这儿并不是直接拆开，而是做了一个线性映射。
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        # (B, S, D) -proj-> (B, S, D_new) -split-> (B, S, Head, W) -trans-> (B, Head, S, W)  # W是一个头的宽度
        #           线性变换               拆成多头
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):  # 编码层输入的是输入向量和pad_mask矩阵。
        """E
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):  # 输入是掩码，输出的att是概率。
        """
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(src_vocab_size, d_model)  # token Embedding用于将离散的标签映射到连续的向量空间，src_vocab_size表示词汇表大小， d_model每个嵌入向量的维度。
    ...
    def forward(self, enc_inputs):
        ...
        enc_outputs = self.src_emb(
            enc_inputs)  # [batch_size, src_len, d_model]，先映射，为每一个token对应512维语义。
        ...
        enc_self_attn_mask = get_attn_pad_mask(
            enc_inputs, enc_inputs)  # [batch_size, src_len, src_len]是一个在pad token处被标记为True的三维列表，其中第二个维度是通过广播拓展而来。
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        dec_outputs = self.tgt_emb(
            dec_inputs)  # [batch_size, tgt_len, d_model]  # 为每个词表添加一个向量维度
        ...
        # Decoder中把两种mask矩阵相加（既屏蔽了pad的信息，也屏蔽了未来时刻的信息）返回的上三角矩阵中，对角线元素全部为零， dec_self_attn_subsequence_mask矩阵包含0与1，dec_self_attn_pad_mask是一个布尔值矩阵。布尔值实际上是可以被当作整数来处理的。在整数上下文中，True 等于 1，而 False 等于 0
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder().to(device)  # 输入为encoder_input，输出为encoder_output
        self.decoder = Decoder().to(device)  # 输入为encoder_input、encoder_output和decoder_input
        self.projection = nn.Linear(
            d_model, tgt_vocab_size, bias=False).to(device)  # 每一个token映射到词库中对应token的概率。
...
def greedy_decoder(model, enc_input, start_symbol):
    ...
    dec_input = torch.zeros(1, 0).type_as(enc_input.data)  # 张量的data属性即是将张量的数据部分提取了出来，该部分无法求导，只能当做数值。
    terminal = False
    next_symbol = start_symbol
    while not terminal:
        # 预测阶段：dec_input序列会一点点变长（每次添加一个新预测出来的单词）
        dec_input = torch.cat([dec_input.to(device), torch.tensor([[next_symbol]], dtype=enc_input.dtype).to(device)],
                              -1)  # 因为decoder_input的输出是一个token一个token逐个预测的，所以刚开始的时候以start_token作为开头。
        dec_outputs, _, _ = model.decoder(dec_input, enc_input, enc_outputs)#1*7*512，当前有几个值就预测几个值。
        projected = model.projection(dec_outputs)  # 线性映射1*7*12
        prob = projected.squeeze(0).max(dim=-1, keepdim=False)[1]#选择最大的概率
    ...
    #     -1)
    greedy_dec_predict = dec_input[:, 1:]  # 拿出当前对象中的所有序列
...
for i in range(len(enc_inputs)):
    ...
    print([src_idx2word[t.item()] for t in enc_inputs[i]], '->',
          [idx2word[n.item()] for n in greedy_dec_predict.squeeze()])#item取单个值，而且精度更高
