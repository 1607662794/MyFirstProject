# https://blog.csdn.net/shazam886/article/details/123338800
# 两个任务各有各的标签，MLM中标签为原token，NSP中标签为0,1.
import os
import random
...
def dictionary(sentences):  # 对于不是特殊字符的字符，使用顺序索引来对其进行编码
...
def make_batch(sentences, batch_size):
    input_ids = []
    segment_ids = []
    masked_token = [] # 掩码的元素是token化后的值
    masked_position = [] # 进行掩码的位置。
    IsNext = []
    positive = negative = 0
    while positive != batch_size / 2 or negative != batch_size / 2:
        ...
        seg_ids = [0] * (len(sentences[num_a]) + 2) + [1] * (len(sentences[num_b]) + 1)  # 语义标签，前一个句子加一个CLS和SEP均为0，后一个句子加SEP为1。

        mask_num = min(max_mask, math.ceil(len(token_ids) * 0.15))  # 向上取整，随机选择句子对中的15%的token进行掩码。
        mask_token, mask_position = [], []
        for i in range(mask_num):  # 有了15%比例后，就相当于有了随机挑选的次数，然后进行操作。
            while True:
                num = randint(0, len(token_ids) - 1)
                if token_ids[num] != 0 and token_ids[num] != 1 and token_ids[num] != 2 and token_ids[num] != 3:
                    break
            mask_token.append(token_ids[num])  # 保留进行掩码处理的元素。
            if randint(0, 1) < 0.8:
                token_ids[num] = 3  # 这部分掩码被替换为[mask]
            elif randint(0, 1) < 0.5:
                while True:
                    num1 = randint(0, len(token_ids) - 1)#从当前句子对中，摘出来一个token，用来替换，
        ...
        token_ids = token_ids + [0] * temp_token#不够max_max_len的部分用0token来进行填充。
        seg_ids = seg_ids + [0] * temp_token#不够max_max_len的部分用0token来进行填充。但是不等于句子1.
        temp_mask = max_mask - len(mask_token)#最少得有5个mask处，其余部分依然采用填充的形式。
    ...
    return torch.LongTensor(input_ids), torch.LongTensor(segment_ids), torch.LongTensor(masked_token), torch.LongTensor(
        masked_position), torch.LongTensor(IsNext)
...
class Embedding(nn.Module):
    def __init__(self):
        ...
        self.token_embed = nn.Embedding(len_vocab, Embedding_size)#将词汇表长度映射到编码长度768
        self.position_embed = nn.Embedding(seq_max_len, Embedding_size)#默认的序列最大长度，可以看做每次可以处理的token的最大长度。
        self.seg_embed = nn.Embedding(len_vocab, Embedding_size)#词汇表的最大长度
        self.norm = nn.LayerNorm(Embedding_size)

    def forward(self, input_ids, segment_ids):
        position = np.arange(0, len(input_ids[0]))#他这儿的位置编码比较简单，直接按照位置来了。
        position = torch.LongTensor(position)
        position = position.unsqueeze(0).expand(6, 30)
        token_embed = self.token_embed(input_ids)#token也比较简单，直接按照序号来安排了。
...
class MLM(nn.Module):
    ...
    def forward(self, en_output, masked_position):
        masked_position = masked_position.unsqueeze(1).expand(batch_size, Embedding_size, max_mask).transpose(1,
                                                                                                              2)  # [6,5,768]
...
if __name__ == '__main__':
    ...
    sentences_ids = []
    for i in range(len(sentences)):  # 将sentences中的字符转化为索引，从而构成一个新的序列。
        temp = []
    ...
    for epoch in range(100):
        ...
        loss_cls = criterion(cls_output, IsNext)#两个任务，然后将两个任务的损失函数直接相加，得到总的损失函数。
